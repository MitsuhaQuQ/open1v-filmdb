...
- `view` opens the database browser at the current month. Dates and rolls use numbered menus; `p` and `n` change month immediately without Enter, and `q` returns to the preceding menu. A selected roll displays decoded fields as `Field: value` entries separated by `|`; missing values display as `n/a`.
