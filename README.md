...
- `set` reads the camera's current shooting-data field mask and lists all 18 selectable recorded fields as a numbered ON/OFF menu. Selecting a field offers numbered ON/OFF choices and requires `y` confirmation before writing. Composite bits, field dependencies, and the 28-byte optional-data limit are validated before the E7/E9 write, and E8 read-back verifies the result. Changing this setting while a film is partly exposed can create a new logical roll segment.
