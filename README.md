...
- `set` reads the camera's current shooting-data field mask and lists all 18 selectable recorded fields as a numbered ON/OFF menu. Every item shows its byte cost, and the final menu line shows current optional-data usage out of 28 bytes plus the 8/16/32-byte internal record length. Entering an item number toggles it immediately in the staged preview; `*` marks changes. Use `commit` for one verified E7/E9 write or `discard`/`q` to abandon the staged changes. An item that would exceed the limit cannot be enabled. Composite bits and field dependencies are validated, and E8 read-back verifies a commit.
- `stop` sends the session-ending exchange and releases the active camera connection. Interactive `sync`, `clear`, and `set` deliberately keep the same camera session active; run `stop` before `exit`.
